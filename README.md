...
- `words_travel.json`：旅遊主題單字庫（200個，N5為主+少量N4/N3，已逐字檢查讀音/意思）
- `words_menu.json`：菜單/餐廳主題單字庫（200個，N5為主+少量N4/N3）
...
每天訊息底部會顯示旅遊、菜單各自的「第 X / Y 天」（200字 ÷ 10 = **20天一輪**）。
其中第1~10天是第一批100字，第11~20天自動換成第二批100字，**不需要手動更新單字庫**。

20天跑完一輪後會自動從頭重新開始（用同一批200字複習），同時訊息會提示：
「200字已全部學完！可以考慮升級到 N4 單字庫了 🎓」。

如果之後想升級到N4內容，可以請Claude準備N4版本的旅遊/菜單單字庫並調整程式。
