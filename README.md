...
每天 10:00 推送「旅遊」4個 + 「美食」8個（共12個）單字，
...
- `send_words.py`：10:00推送，旅遊4個＋美食8個新單字，並檢查PAT是否快過期
- `send_review.py`：22:00推送，複習今天學過的單字
- `.github/workflows/schedule.yml`：排程定義（UTC 02:00 / UTC 14:00）

## 進度顯示與輪換機制

每天訊息底部會顯示旅遊、美食各自的「第 X / Y 天」：
- 旅遊：120字 ÷ 4個/天 = **30天一輪**
- 美食：160字 ÷ 8個/天 = **20天一輪**

兩者各自獨立輪換（天數不同沒關係），單字庫依「先N5、後N4、後N3」排序，
讓學習從基礎逐步進階，**不需要手動更新單字庫**。

一輪跑完後會自動從頭重新開始（用同一批單字複習），同時訊息會提示
「這輪全部學完！明天將從頭開始複習 🔄」。
