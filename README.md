...
- `words_travel.json`：旅遊主題單字庫（100個，N5為主+少量N4/N3，已逐字檢查讀音/意思）
- `words_menu.json`：菜單/餐廳主題單字庫（100個，N5為主+少量N4/N3）
...
每天訊息底部會顯示旅遊、菜單各自的「第 X / Y 天」（100字 ÷ 10 = **10天一輪**）。
跑完一輪會自動從頭重新開始（用同一批100字複習），**不需要手動更新單字庫** ——
這100字已經是N5旅遊/餐廳情境下的完整精選詞彙。

如果之後想要「每輪都是全新內容」而非重複複習，可以再請Claude擴充第二輪的100字
（屆時會調整程式邏輯，讓兩輪內容不同）。
